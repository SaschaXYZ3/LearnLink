...
import { Container, Button, Form, Modal, Card, ListGroup, Badge } from "react-bootstrap";
...
const TutorView = () => {
    ...
    const [bookings, setBookings] = useState([
        ...
        { studentName: "Jane Smith", course: "React Fundamentals", status: "Pending" }
    ]);
    ...
    const categories = {
        ...
        "IT Security": ["Penetration Testing", "Network Security", "Ethical Hacking"],
                    ...
                    {courses.map((course, index) => (
                        <Card key={index} style={{ width: '20rem' }} className="course-card">
                            <Card.Body>
                                ...
                                <Button variant="danger" size="sm" onClick={() => deleteCourse(index)}>
                ...
                <ListGroup>
                    {bookings.map((booking, index) => (
                        <ListGroup.Item key={index} className="d-flex justify-content-between align-items-center">
                            <div>
                                <strong>{booking.studentName}</strong> requested to join <em>{booking.course}</em>
                            </div>
                            ...
                            {booking.status === "Pending" && (
                                <>
                                    <Button variant="success" size="sm" onClick={() => acceptBooking(index)}>
                                        <FontAwesomeIcon icon={faUserCheck} /> Accept
                                    </Button>
                                    <Button variant="danger" size="sm" onClick={() => rejectBooking(index)}>
                            ...
                            {booking.status === "Accepted" && (
                                <>
                                    <Button variant="secondary" size="sm" onClick={() => moveToPending(index)}>
                                        <FontAwesomeIcon icon={faArrowLeft} /> Move to Pending
                                    </Button>
                                    <Button variant="danger" size="sm" onClick={() => removeBooking(index)}>
